...
Fomally, you should compute the maximum value of 
Bi + Bi+1 + Bi+2 + ... + Bj, where 0 = i = j < N · K.
...
A0, A1, ..., AN-1.
Output
For each test case, print a single line containing the maximum 
subarray sum of B.
...
2
Explanation
Example case 1: B = {1, 2, 1, 2, 1, 2} and the 
subarray with maximum sum is the whole {1, 2, 1, 2, 1, 2}. 
...
ll maxSubarraySum(int arr[],int n,int k){
	...
	if(total_sum>0){
		...
		ans=max(max_prefix_sum+max_prefix_sum+(total_sum*(k-2)),kadane_sum);
	}
	return ans;
}
